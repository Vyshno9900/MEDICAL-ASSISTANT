...
app = FastAPI(title="IMDS Capstone (No Login)", version="1.0.0")

# Static: mount only if the folder exists (prevents startup crash)
...
def about(request: Request):
    return templates.TemplateResponse(
        request=request,
        name="about.html",
        context={"request": request, "model": GEMINI_MODEL, "disclaimer": DISCLAIMER},
    )
